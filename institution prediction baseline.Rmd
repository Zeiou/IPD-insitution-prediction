...
#remove outlier in CamPalGN and PINE

boxplot(data5y.1v$agebl, plot=FALSE)$out
...
model1$res[c("gamma0", "gamma1", "gamma2"), "est"]
...
data5y.1v$pred.re<-1/(1+exp(-17.5665135-0.9346098+0.9821521*log(4)-0.4243082*data5y.1v$rcs2+data5y.1v$lp.1.re))

(1-obj.1$surv)/mean(1-data5y.1v$pred.re)
...
log((1-obj.6$surv)/mean(1-data5y.6v$pred)) #ln mean O/E risk ratio 
...
data5y.6v$lp.1.re <- as.vector(as.matrix(des_matr6) %*% cbind(coef6.re))

data5y.6v$pred.re<-1/(1+exp(-21.7801977+0.3555012+0.7125373*log(4)-0.7653505*data5y.6v$rcs2+data5y.6v$lp.6))

(1-obj.6$surv)/mean(1-data5y.6v$pred.re)
