...
#From the calibration plot, we can see the heterogeneity between studies'
#Check the population first, age group and the number event.
#Divide by 3 group first


summary(imp3.5y$age10) # Min 3.750, Max 9.649

imp3.5y%>%
  filter(cens==1)%>%
  group_by(clus)%>%
  summarise(min(age10),median(age10),max(age10)) #One patient in PINE has event 45.8 years old, maybe an outlier,
                                                 # idpicc is 1305


data5y.event<-imp3.5y%>%
              filter(cens==1)
...
#mean predicted risk 
#exp_t.1 <-mean(1-data5y.1v$pred)

cf1<-log(((obj.fre.1)/(1-obj.fre.1))/((exp_t.1)/(1-exp_t.1))) #correction factor  -1.07304


data5y.1v$pred<-predict(model1,newdata = data5y.1v,type = "survival",times = 4)
data5y.1v$g_st<-log((1/data5y.1v$pred)-1)
val.1<-lm(g_st~lp.1,data = data5y.1v)
summary(val.1) #But this is -14.14??

model1

gamma.m1 <- model1$res[c("gamma0", "gamma1", "gamma2"), "est"]

rcs.mod1<-as.data.frame(basis(model1$knots,log(data5y.1v$year))) #basis spline

data5y.1v$rcs1<-rcs.mod1[,2]
   
data5y.1v$rcs2<-rcs.mod1[,3]

data5y.1v$pred.re<-1/(1+exp(-17.5665135+0.9822*log(data5y.1v$year)-0.4243082*data5y.1v$rcs2+data5y.1v$lp.1)) #survival probabilities

data5y.1v$risk.re<-1-data5y.1v$pred.re

model1.re<-flexsurvspline(Surv(year,cens)~age10+sex+mdsupdrs3.10+hybl,data=data5y.1,k=1,bknots = c(log(1.147159),log(3.761807)),scale = "odds")

data5y.1v$pred.re<-predict(model1.re,newdata = data5y.1v,type = "survival",times = 4)[[2]]
...
coef1.re<-c(model1$coefficients[4],model1$coefficients[5],model1$coefficients[6],model1$coefficients[7])
data5y.1v$lp.1.re <- as.vector(as.matrix(des_matr1) %*% cbind(coef1.re))

rcs.mod1<-as.data.frame(basis(model1$knots,log(4))) #basis spline
data5y.1v$rcs1<-rcs.mod1[,2]
data5y.1v$rcs2<-rcs.mod1[,3]
data5y.1v$pred.re<-1/(1+exp(-17.5665135-1.07304+0.9822*log(4)-0.4243082*data5y.1v$rcs2+data5y.1v$lp.1.re))
data5y.1v$risk.re<-1-data5y.1v$pred.re
...
cf6<-log(((obj.fre.6)/(1-obj.fre.6))/((exp_t.6)/(1-exp_t.6))) #correction factor  
...
data5y.6v$pred.re<-1/(1+exp(-21.780+cf6+0.713*log(data5y.6v$year)-0.765 *log(data5y.6v$year)+data5y.6v$lp.6)) #recalibration survival probabilities
...
# I am not sure if this is actual survival? data5y.1v$pred.act<-predict(vcal.1.RP,type = "survival",times = 4,newdata = data5y.1v)[[2]]  


#Try estimate survivial by model development from validation dataset to get the actual survival

model.val.1<-flexsurvspline(Surv(year,cens)~age10+sex+mdsupdrs3.10+hybl,data=data5y.1v,k=1,bknots = c(log(1.147159),log(3.761807)),scale = "odds")

data5y.1v$pred.act<-predict(model.val.1,type = "survival",times = 4,newdata = data5y.1v)[[2]] #actual survival
...
calslope_summary.1 <- c(
  ...
  "2.5 %"  = val.1$coefficients[2] - 1.96 * 0.04585,
  "97.5 %" = val.1$coefficients[2] + 1.96 * 0.04585
)
...
calslope_summary.2 <- c(
  ...
  "2.5 %"  = val.2$coefficients[2] - 1.96 * 0.001268,
  "97.5 %" = val.2$coefficients[2] + 1.96 * 0.001268
)
...
calslope_summary.5 <- c(
  ...
  "2.5 %"  = val.5$coefficients[2] - 1.96 * 0.05232,
  "97.5 %" = val.5$coefficients[2] + 1.96 * 0.05232
)
...
data5y.6v$pred.act<-predict(model.val.6,type = "survival",times = 4,newdata = data5y.6v)[[2]]
...
calslope_summary.6 <- c(
  ...
  "2.5 %"  = val.6$coefficients[2] - 1.96 * 0.0210,
  "97.5 %" = val.6$coefficients[2] + 1.96 * 0.0210
)
