...
#linear predictor (to double check if the equation is right)

#data5y.1v$coef1<-model1$coefficients[4]
#data5y.1v$coef2<-model1$coefficients[5]
#data5y.1v$coef3<-model1$coefficients[6]
#data5y.1v$coef4<-model1$coefficients[7]

#data5y.1v%>%
#  mutate(lp=age10*coef1+sex*coef2+mdsupdrs3.10*coef3+hybl*coef4)
...
data5y.1v$lp.1<-lp.1 #It is the same as lp!!!
...
h0.2<-predict(model2,newdata = data5y.2v,type="hazard",times=4)  #H0(t=4)
h.2<-predict(model2,newdata = data5y.2v,type="cumhaz",times=4)   #H(t=4)


lp.2<-log((exp(h.2[[2]])-1)/(exp(h0.2[[2]])-1))
...
h0.3<-predict(model3,newdata = data5y.3v,type="hazard",times=4)  #H0(t=4)
h.3<-predict(model3,newdata = data5y.3v,type="cumhaz",times=4)   #H(t=4)

lp.3<-log((exp(h.3[[2]])-1)/(exp(h0.3[[2]])-1))
...
h0.4<-predict(model4,newdata = data5y.4v,type="hazard",times=4)  #H0(t=4)
h.4<-predict(model4,newdata = data5y.4v,type="cumhaz",times=4)   #H(t=4)

lp.4<-log((exp(h.4[[2]])-1)/(exp(h0.4[[2]])-1))
...
h0.5<-predict(model5,newdata = data5y.5v,type="hazard",times=4)  #H0(t=4)
h.5<-predict(model5,newdata = data5y.5v,type="cumhaz",times=4)   #H(t=4)

lp.5<-log((exp(h.5[[2]])-1)/(exp(h0.5[[2]])-1))
...
h0.6<-predict(model6,newdata = data5y.6v,type="hazard",times=4)  #H0(t=4)
h.6<-predict(model6,newdata = data5y.6v,type="cumhaz",times=4)   #H(t=4)

lp.6<-log((exp(h.6[[2]])-1)/(exp(h0.6[[2]])-1))
...
#flexsurvspline(Surv(year,cens)~lp.1,data=data5y.1v,k=1,bknots = c(log(0.1067762),log(4.939083)),scale = "odds")
...
val.4 <- coxph(Surv(year, cens) ~ lp.4, data = data5y.4v)

calslope_summary.4 <- c(
  "calibration slope" = val.4$coef,
  "2.5 %"  = val.4$coef - qnorm(1 - alpha / 2) * sqrt(val.4$var),
  "97.5 %" = val.4$coef + qnorm(1 - alpha / 2) * sqrt(val.4$var)
)
...
val.5 <- coxph(Surv(year, cens) ~ lp.5, data = data5y.5v)

calslope_summary.5 <- c(
  "calibration slope" = val.5$coef,
  "2.5 %"  = val.5$coef - qnorm(1 - alpha / 2) * sqrt(val.5$var),
  "97.5 %" = val.5$coef + qnorm(1 - alpha / 2) * sqrt(val.5$var)
)
...
val.6 <- coxph(Surv(year, cens) ~ lp.6, data = data5y.6v)

calslope_summary.6 <- c(
  "calibration slope" = val.6$coef,
  "2.5 %"  = val.6$coef - qnorm(1 - alpha / 2) * sqrt(val.6$var),
  "97.5 %" = val.6$coef + qnorm(1 - alpha / 2) * sqrt(val.6$var)
)
...
#log(0.1067762),log(3.978097)

#val.4.RP <- flexsurvspline(Surv(year, cens) ~ lp.4, data = data5y.4v, k=1,bknots = c(log(0.1067762),log(3.978097)),scale = "odds")


#Error is the ar = c(gamma0 = -248.680716918727, gamma1 = -3615.19526399349, : initial value in 'vmmin' is not finite


val.5.RP <- flexsurvspline(Surv(year, cens) ~ lp.5, data = data5y.5v, k=1,bknots = c(log(0.5557837),log(3.978097)),scale = "odds")


calslope_summary.5.RP <- c(
  "calibration slope" = 7.546 ,
  "2.5 %"  = 7.546  - qnorm(1 - alpha / 2) * 1.860,
  "97.5 %" = 7.546  + qnorm(1 - alpha / 2) * 1.860
)

calslope_summary.5.RP


val.6.RP <- flexsurvspline(Surv(year, cens) ~ lp.6, data = data5y.6v, k=1,bknots = c(log(0.1067762),log(3.8987)),scale = "odds")


  calslope_summary.6.RP <- c(
  "calibration slope" = 2.4704 ,
  "2.5 %"  = 2.4704  - qnorm(1 - alpha / 2) * 0.8303,
  "97.5 %" = 2.4704  + qnorm(1 - alpha / 2) * 0.8303
)

calslope_summary.6.RP
...
vcal.5.RP <- flexsurvspline(Surv(year, cens) ~ risk.cll, data = data5y.5v, k=1,bknots = c(log(0.5557837),log(3.978097)),scale = "odds")
...
vcal.6.RP <- flexsurvspline(Surv(year, cens) ~ risk.cll, data = data5y.6v, k=1,bknots = c(log(0.1067762),log(3.8987)),scale = "odds")
