...
```{r not run try cox secondary model}

#---leave CamPalGN out----


vcal.1.10.RP <- cph(Surv(year, cens) ~ rcs(risk.cll, 3), x = T, y = T,surv = T,data = data10y.1v)


#survest function is to estimate survival probabilities

dat_cal.1.10 <- cbind.data.frame(
                   "obs" = 1 - rms::survest(vcal.1.10.RP,
                    times = 7,
                    newdata = data10y.1v)$surv, #1-s(t=4) to get the refitted model predicted risk
  
  "lower" = 1 - rms::survest(vcal.1.10.RP,
                        times = 7,
                        newdata = data10y.1v)$upper,
  
 "upper" = 1 - rms::survest(vcal.1.10.RP,
                            times = 7,
...
```{r recalibration expect PICNICS}
...
data10y.4v$pred.re.1<-1/(1+exp(-16.3312278-0.03665086-1.27+1.8903119*data10y.4v$rcs1-0.9449542*data10y.4v$rcs2+1.1*data10y.4v$lp.4.10)) #change the knots position and s(lnt;r) but keep the linear predictor the same
...
data10y.5v$pred.re.1<-1/(1+exp(-16.166035-0.4202508-2.06+1.372743*data10y.5v$rcs1-0.328912*data10y.5v$rcs2+1.2*data10y.5v$lp.5.10)) #change the knots position and s(lnt;r) but keep the linear predictor the same
